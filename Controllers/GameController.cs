...
namespace TamboliyaApi.Controllers
{
    ...
    public class GameController : ControllerBase
    {
        ...
        public async Task<ActionResult<IEnumerable<GameDTO>>> GetLastGamesToJoin()
        {
            ...
            List<Game> userGamesWithUserAtUserInChildGame = new List<Game>();

            if (parentUserGames.Count() == 0)
                return new BadRequestObjectResult("User games not found");
            else
            {
                userGamesWithUserAtUserInChildGame = (await unitOfWork.GameRepository
                .GetAsync(
                filter: u => u.CreatorGuid == userGuid &&
            ...
            if (userGamesWithUserAtUserInChildGame.Count() > 0)
            {
                parentUserGames = parentUserGames.Where(g => userGamesWithUserAtUserInChildGame.All(u => u.ParentGameId != g.Id)).ToList();
            }

            if (parentUserGames.Count() == 0)
                return new BadRequestObjectResult("User games not found");

            var gameDTO = parentUserGames!.Select(x => x.GameToGameDTO()!);
            return Ok(gameDTO);
        }
